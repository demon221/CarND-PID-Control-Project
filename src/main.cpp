...
int main()
{
  ...
  //steer controller
  PID pidSteer;
  pidSteer.Init(0.1, 0.0, 5);

  //throttle controller
  PID pidThrottle;
  pidThrottle.Init(0.1, 0.001, 0.1);
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          //normalization
          steer_value = std::min(std::max(steer_value, -1.0), 1.0);

          //throttle controller
          double speed_target = 40.0 * (1.- std::fabs(steer_value)) + 20;
          double speed_cte = speed_target - speed;
          pidThrottle.UpdateError(speed_cte);
          throttle_value = pidThrottle.TotalError();
          //normalization
          throttle_value = std::min(std::max(throttle_value, -1.0), 1.0);
          
          // DEBUG
          std::cout << "Angle CTE: " << cte << " Steering Value: " << steer_value << std::endl;
